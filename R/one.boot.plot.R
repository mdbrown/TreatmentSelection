...
function(x, ci, fixed.values, fix.ind, out.ind){
  ...
  myboot.sample <- x$functions$boot.sample( x$derived.data[[as.character(x$formula[[2]])]], 
  ...
  if(x$model.fit$link == "risks_provided"){
    ...
    obsrisk.t0.b  <-  get.risk.t0(coef, x$formula, x$treatment.name, data = x$derived.data[ind,], linkinvfun)
    obsrisk.t1.b  <-  get.risk.t1(coef, x$formula, x$treatment.name, data = x$derived.data[ind,], linkinvfun)
  }
...
one.boot.plot_disc <-
  function(x){
   ...
   # browser()
    ...
    ####
    unique.fitted.vals <- unique(cbind(marker.b, tmp.trtsel$derived.data[,4:6]))
...
one.boot.plot.compare <-
  function(data1, data2, formulas,  event.names, treatment.names, ci,
    ...
    myboot.sample <- obp.boot.sample( data1[[event.names[1]]], data1[[treatment.names[1]]], rho)
    
    rho.b <- myboot.sample[1:7]
    ind   <- myboot.sample[-c(1:7)]
    
    event.b <- data1[[event.names[1]]][ind]
