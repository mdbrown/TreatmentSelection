...
function(event, trt, marker, rho = rho, study.design, obp.boot.sample, obp.get.F, fixed.values, fix.ind, out.ind, link){
  ...
  obsrisk.t0.b  <-  get.risk.t0(coef,  marker.b, linkinvfun)#c(ifelse(coef[3]>0, 0,1),        get.risk.t0(coef,  marker.b, linkinvfun))
  obsrisk.t1.b  <-  get.risk.t1(coef,  marker.b, linkinvfun)#c(ifelse(sum(coef[3:4])>0, 0,1), get.risk.t1(coef,  marker.b, linkinvfun))

  obsdelta.b <- obsrisk.t0.b - obsrisk.t1.b#c(-1, obsrisk.t0.b[-1] - obsrisk.t1.b[-1])

  F.Y <- obp.get.F( marker.b,        event.b, trt.b, rho.b)#c(0, obp.get.F( marker.b,        event.b, trt.b, rho.b))
  F.D <- obp.get.F( obsdelta.b, event.b, trt.b, rho.b)#c(0, obp.get.F( obsdelta.b[-1], event.b, trt.b, rho.b))  
