...
plot(trtsel.Yc, bootstraps = 25, 
     show.marker.axis = FALSE)
calibrate(trtsel.Yc, plot.type = "calibration")
calibrate(trtsel.Yc, plot.type = "treatment effect")


evaluate(trtsel.Yc, bootstraps = 25)

trtsel.Y2c <- trtsel(event.c ~ (Y1+Y2)*trt, 
                    treatment.name = "trt", 
                    data = tsdata, 
                    study.design = "RCT", 
                    default.trt = "trt all")

compare(x = trtsel.Yc, x2= trtsel.Y2c, bootstraps = 35)
```
...
trtsel.Ysurv <- trtsel(Surv(xi, di) ~ Y*trt, 
                    treatment.name = "trt", 
                    data = surv_tsdata,  #sample data set with time-to-event outcome
                    prediction.time = 4, #landmark prediction time must be set
                    study.design = "RCT", 
                    default.trt = "trt all")

trtsel.Ysurv #notice 'censoring weights' can now be found in 'derived.data' 

plot(trtsel.Ysurv, bootstraps = 10, 
     show.marker.axis = FALSE)
calibrate(trtsel.Ysurv, plot.type = "calibration")
calibrate(trtsel.Ysurv, plot.type = "treatment effect")


evaluate(trtsel.Ysurv, bootstraps = 10)

trtsel.Y2surv <- trtsel(Surv(xi, di) ~ Y^2 + (Y)*trt, 
                    treatment.name = "trt", 
                    data = surv_tsdata, 
                    prediction.time = 4, 
                    study.design = "RCT", 
...
compare(x = trtsel.Ysurv, x2= trtsel.Y2surv, bootstraps = 10)
```
